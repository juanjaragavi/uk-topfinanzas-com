...
async function runTests() {
  ...
  results.withoutUTM = await testScenario('Submission WITHOUT UTM Parameters', testWithoutUTM);
  
  await new Promise(resolve => setTimeout(resolve, 2000));
  
  // Test WITH UTM (the problematic case)
  results.withUTM = await testScenario('Submission WITH UTM Parameters', testWithUTM);
  
  // Final summary
  console.log('\n' + '='.repeat(60));
  console.log('📊 TEST RESULTS SUMMARY');
  console.log('='.repeat(60));
  console.log(`Without UTM Parameters: ${results.withoutUTM ? '✅ PASSED' : '❌ FAILED'}`);
  console.log(`With UTM Parameters:    ${results.withUTM ? '✅ PASSED' : '❌ FAILED'}`);
